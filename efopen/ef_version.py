...
def handle_args_and_set_context(args):
    ...
    parser.add_argument("--limit",
                        help="Limit 'history', 'rollback', 'show' to first N records (default 100, max 1000)",
    ...
    if EFConfig.ALLOW_EF_VERSION_SKIP_PRECHECK:
        parser.add_argument("--noprecheck", help="--set or --rollback without precheck", action="store_true",
                            default=False)
...
def precheck_dist_hash(context):
    ...
    try:
        response = requests.get(current_dist_hash.location, timeout=5)
        if response.status_code != 200:
            raise IOError("Non-200 response " + str(response.status_code) + " reading " + current_dist_hash.location)
        dist_hash_in_service = response.text.strip()
...
def get_version_by_value(context, value):
    ...
    for version in versions:
        if version.value == value:
            return version
    fail(
        "Didn't find a matching version for: "
...
def cmd_set(context):
    ...
    if context.value == "=prod":
        ...
        if func_name in globals() and isfunction(globals()[func_name]):
            ...
            raise RuntimeError(
                "{} version for {}/{} is '=latest' but can't look up because method not found: {}".format(
