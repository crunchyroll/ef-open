...
def format_secret(secret, version=1):
    ...
    if version == 2:
        return 'kms:{}'.format(secret)
    return "{{{{aws:kms:decrypt,{}}}}}".format(secret)
...
def generate_secret(length=32):
    ...
    indices = (len(alphabet) * b // 256 for b in bytearray(random_bytes))
    return "".join([alphabet[index] for index in indices])


def generate_secret_file(file_path, pattern, service, environment, clients, version=1):
    """
    ...
    try:
        for key, value in data["params"][environment].items():
            if pattern in key:
                if "aws:kms:decrypt" in value or value.startswith("kms:"):
                    ...
                    data["params"][environment][key] = format_secret(encrypted_password, version)
                    changed = True
    except KeyError:
...
def handle_args_and_set_context(args):
    ...
    parser.add_argument("--match", help="used in conjunction with --secret_file to match against keys to be encrypted",
                        default="")
    parser.add_argument("--length", help="length of generated password (default 32)", default=32)
    parser.add_argument("--lbv1_escapes", help="if secret has escapes, encode them for lbv1 consumption",
                        action="store_true", default=False)
    parser.add_argument("--lbv", help="LB output format version", default=1, type=int)
    ...
    if not parsed_args["lbv1_escapes"]:
        ...
        context.plaintext = strip_string_escape(parsed_args["plaintext"])
    context.lb_version = parsed_args["lbv"]
...
def strip_string_escape(str_in):
    if sys.version_info[0] >= 3:
        return str_in.encode('latin1').decode('unicode_escape')
    return str_in.decode('string_escape')
...
def main():
    ...
    if context.re_encrypt:
        ...
        print(format_secret(encrypted_password, context.lb_version))
        return
