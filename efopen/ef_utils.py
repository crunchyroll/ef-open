...
def http_get_metadata(metadata_path, timeout=__HTTP_DEFAULT_TIMEOUT_SEC):
    ...
    RAISE:
      IOError if there was a problem reading metadata
    """
    metadata_path = __METADATA_PREFIX + metadata_path
    try:
        response = requests.get(metadata_path, timeout=timeout)
        if response.status_code != 200:
            raise IOError("Non-200 response %d reading %s" % (response.status_code, metadata_path))
...
def is_in_virtualbox():
    ...
    try:
        virtual_type = str(
            subprocess.check_output(["sudo", "-n", __VIRT_WHAT]).decode('UTF-8', 'ignore')
        ).split('\n')[0:2]
        return virtual_type == __VIRT_WHAT_VIRTUALBOX_WITH_KVM
...
def whereami():
    ...
    if getenv("JENKINS_URL") and getenv("JENKINS_DOCKER") is None:
        # The addition of the JENKINS_DOCKER is a temporary workaround to have Jenkins Docker machine rely on its
        # instance role and assume roles vs a credentials file. This is an on-going effort to move everything to code
        # with https://ellation.atlassian.net/browse/OPS-13637
...
def kms_decrypt(kms_client, secret):
    ...
    """
    try:
        matches = re.fullmatch(r"\{\{aws:kms:decrypt,([^}]+)\}\}", secret) or re.fullmatch(r"(?:kms:)?(.*)", secret)
        print(matches)
        if matches is not None:
            secret = matches[1]
        response = kms_client.decrypt(CiphertextBlob=base64.b64decode(secret))
        decrypted_secret = DecryptedSecret(response["Plaintext"].decode('UTF-8', 'ignore'), response["KeyId"])
...
def kms_key_alias(kms_client, key_arn):
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to obtain key alias for arn {}, error: {}".format(key_arn, error.response["Error"]["Message"]))
...
def kms_key_arn(kms_client, alias):
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to obtain key arn for alias {}, error: {}".format(alias, error.response["Error"]["Message"]))
...
def get_autoscaling_group_properties(asg_client, env, service):
    """
    Gets the autoscaling group properties based on the service name that is provided. This function will attempt to find
    the autoscaling group based on the following logic:
    ...
    Args:
      asg_client: Instantiated boto3 autoscaling client
    ...
    try:
        ...
        if len(response["AutoScalingGroups"]) == 0:
            ...
            response = asg_client.describe_tags(Filters=[{"Name": "Key", "Values": ["Name"]},
                                                         {"Name": "Value", "Values": ["{}-{}".format(env, service)]}])
...
def typed_property(cls, default=None):
    def wrapper(def_gen):
        @functools.wraps(def_gen)
        def getter(self):
            if def_gen.__name__ not in self.__dict__:
                return def_gen(self)
            return self.__dict__.get(def_gen.__name__)

        def setter(self, value):
            if type(value) is not cls:
                raise TypeError("{} value must be {}".format(def_gen.__name__, cls.__name__))
            self.__dict__[def_gen.__name__] = value
