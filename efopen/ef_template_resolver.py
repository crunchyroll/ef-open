...
class EFTemplateResolver(object):
    ...
    def __init__(self,
        ...
        # target is "other"
            ...
            try:
                if whereami() == "ec2":
                    self.resolved["ACCOUNT"] = str(
                        json.loads(http_get_metadata('iam/info'))["InstanceProfileArn"].split(":")[4])
                else:
    ...
    def load(self, template, parameters=None):
        ...
        elif hasattr(template, 'read') and callable(template.read):
            try:
        ...
        if parameters:
            ...
            elif hasattr(parameters, 'read') and callable(parameters.read):
                try:
                    self.parameters = yaml.safe_load(parameters)
                    parameters.close()
                except ValueError as error:
    ...
    def search_parameters(self, symbol):
        ...
        # This lookup is redundant when env_short == env_full, but it's also cheap. Also handles the case for
        # mgmt.<account_alias>
        ...
        while go_again:
            ...
            for symbol in template_symbols:
                ...
                # Lookups in credentials
                    ...
                    # self.resolved[symbol] may have value=None; use has_key tell "resolved w/value=None"
                    # from "not resolved" these may be "global" symbols such like "ENV", "ACCOUNT", etc.
