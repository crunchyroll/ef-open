...
class NewRelicAlerts(object):
    ...
    def update_cloudfront_policy(self):
        ...
        def map_function(x):
            return x['Id'], ', '.join(x['Aliases']['Items'])

        queue = [map_function(it) for it in distribution_list['Items']]

        while distribution_list['IsTruncated']:
            distribution_list = cloudfront.list_distributions(Marker=distribution_list['NextMarker'])[
                'DistributionList']
        ...
        conditions = {}
        for _id, alias in queue:
            conditions['4xx Average {}'.format(alias)] = meta(
                'error4xxErrorRate', 10, _id, '4xx Average {}'.format(alias), policy.id)
            conditions['5xx Average {}'.format(alias)] = meta(
                'error5xxErrorRate', 5, _id, '5xx Average {}'.format(alias), policy.id)
        ...
        for remote_alert_nrql_condition in policy.remote_alert_nrql_conditions:
            if remote_alert_nrql_condition["name"] == local_alert_nrql_condition["name"]:
                ...
                if local_alert_nrql_condition != remote_alert_nrql_condition:
                    logger.info(
                        "Local alert nrql condition differs from remote alert nrql condition for {}-{}. Updating remote.".format(
                            policy.env, policy.service))
                    logger.debug(
                        "Local: {}\nRemote: {}".format(local_alert_nrql_condition, remote_alert_nrql_condition))
                    self.newrelic.put_policy_alert_nrql_condition(remote_alert_nrql_condition["id"],
                                                                  local_alert_nrql_condition)
        ...
        for remote_alert_apm_condition in policy.remote_alert_apm_conditions:
            if remote_alert_apm_condition["name"] == local_alert_apm_condition["name"]:
                ...
                if local_alert_apm_condition != remote_alert_apm_condition:
                    logger.info(
                        "Local alert apm condition differs from remote alert apm condition for {}-{}. Updating remote.".format(
                            policy.env, policy.service))
                    logger.debug("Local: {}\nRemote: {}".format(local_alert_apm_condition, remote_alert_apm_condition))
                    self.newrelic.put_policy_alert_apm_condition(remote_alert_apm_condition["id"],
                                                                 local_alert_apm_condition)
    ...
    def update_application_services_policies(self):
        for service_name, service_config in self.context.service_registry.iter_services(
                service_group="application_services"):
            ...
            if self.context.env in service_environments:
                ...
                remote_alert_nrql_condition_names = [remote_alert_nrql_condition['name'] for remote_alert_nrql_condition
                                                     in policy.remote_alert_nrql_conditions]
                ...
                remote_alert_apm_condition_names = [remote_alert_apm_condition['name'] for remote_alert_apm_condition in
                                                    policy.remote_alert_apm_conditions]
