...
from .ef_utils import create_aws_clients, get_autoscaling_group_properties, fail, typed_property
...
def handle_args_and_set_context(args):
    ...
    parser.add_argument("--verbose", help="Print additional info + resolved template", action="store_true",
                        default=False)
    ...
    parser.add_argument("--percent",
                        help="Specifies an override to the percentage of instances in an Auto Scaling rolling update "
                             "(e.g. 10 for 10%%)",
...
def calculate_max_batch_size(asg_client, service, percent):
    autoscaling_group_properties = get_autoscaling_group_properties(asg_client, service.split("-")[0],
...
def main():
        ...
        for key in modify_template["Resources"]:
            if modify_template["Resources"][key]["Type"] == "AWS::AutoScaling::AutoScalingGroup":
                if "UpdatePolicy" in modify_template["Resources"][key]:
                    ...
                    autoscaling_group_properties = get_autoscaling_group_properties(clients["autoscaling"],
                                                                                    service.split("-")[0],
                                                                                    "-".join(service.split("-")[1:]))
                    new_max_batch_size = calculate_max_batch_size(clients["autoscaling"], service, context.percent)
                    modify_template["Resources"][key]["UpdatePolicy"]["AutoScalingRollingUpdate"][
    ...
    try:
        if context.changeset:
            ...
            results_ids = {key: value for key, value in results.items()
            ...
            if context.poll_status:
                while True:
                    stack_status = clients["cloudformation"].describe_stacks(StackName=stack_name)["Stacks"][0][
                        "StackStatus"]
                    if context.verbose:
