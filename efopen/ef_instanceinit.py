...
def merge_files(service, skip_on_user_group_error=False):
    ...
    if WHERE == "ec2":
        ...
        raise OSError("unsupported environment {}, must be ec2 or virtualbox".format(WHERE))
    ...
    while config_reader.next():
        ...
        if skip_on_user_group_error:
            ...
            try:
                ...
                log_info(
                    "File specifies group {} that doesn't exist in environment. Skipping config file.".format(group))
                continue
        ...
        if not resolver.resolved_ok():
            critical("Couldn't resolve all symbols; template has leftover {{ or }}: {}".format(
...
def main():
    ...
    if WHERE == "virtualbox-kvm":
        ...
        raise OSError("unsupported env {}".format(WHERE))
