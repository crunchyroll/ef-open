...
def handle_args_and_set_context(args):
    ...
    parser.add_argument("--devel", help="Allow running from branch; don't refresh from origin", action="store_true",
                        default=False)
    parser.add_argument("--only_for",
                        help="optional: run ef-generate for a only specific entry in the service registry",
                        default=None)
    ...
    context.policy_template_path = normpath(
        dirname(context.service_registry.filespec)) + EFConfig.POLICY_TEMPLATE_PATH_SUFFIX
...
def resolve_policy_document(policy_name):
    ...
    if CONTEXT.whereami == "ec2":
        ...
        resolver = EFTemplateResolver(profile=CONTEXT.account_alias, env=CONTEXT.env,
        ...
        if not AWS_RESOLVER.ec2_security_group_security_group_id(sg_name):
            ...
            if CONTEXT.commit:
                try:
                    ...
                    fail("Exception creating security group named: {} in VpcId: {}".format(sg_name, vpc_name),
                         sys.exc_info())
...
def conditionally_inline_policies(role_name, sr_entry):
    ...
    for policy_name in sr_entry['policies']:
        ...
        if CONTEXT.commit:
            try:
                CLIENTS["iam"].put_role_policy(RoleName=role_name, PolicyName=policy_name,
        ...
        kms_key_policy = {
            ...
            "Statement": [
                ...
                },
                {
                    ...
                    "Effect": "Allow",
                    "Principal": {
                        "AWS": "arn:aws:iam::{}:role/aws-service-role/autoscaling.amazonaws.com/AWSServiceRoleForAutoScaling".format(
                            account_id)},
                    "Action": [
                        "kms:Encrypt",
                        "kms:Decrypt",
...
def main():
    ...
    try:
        # If running in EC2, always use instance credentials. One day we'll have "lambda" in there too, so
        # use "in" w/ list
        if CONTEXT.whereami == "ec2":
            ...
            CLIENTS = create_aws_clients(EFConfig.DEFAULT_REGION, CONTEXT.account_alias, "cloudfront",
                                         "ec2", "iam", "kms", "sts")
