...
def handle_args_and_set_context(args):
    ...
    parser.add_argument("--no_params", help="disable loading values from params file", action="store_true",
                        default=False)
...
def merge_files(context):
    ...
    if context.no_params is False:
        ...
        if "environments" in dest:
            if not resolver.resolved["ENV_SHORT"] in dest["environments"]:
                print(
                    "Environment: {} not enabled for {}".format(resolver.resolved["ENV_SHORT"], context.template_path))
                return
    ...
    if not resolver.resolved_ok():
        raise RuntimeError(
            "Couldn't resolve all symbols; template has leftover {{ or }}: {}".format(resolver.unresolved_symbols()))
