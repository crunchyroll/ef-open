## What does the crf-open crf-cf tool do?
...
unresolved and processing will stop with an error (crf-cf won't upload a template with unresolved symbols)

Note there may be some scenarios where you need to have {{ }} or {{{ }}} in a string but you aren't intending it
to be a symbol. crf-cf will complain that you have dangling symbols. To fix this just add spaces like so { { } } or { { { } } }
Here's an example
...
## Symbols for CloudFormation templates deployed with crf-cf:
...
Needs: Base64 encoded string encrypted using the unique service/environment kms key generated by crf-password<br>
...
### CRF Config (local config) lookup symbols

#### {{efconfig:accountaliasofenv,\<env>}}
Returns: A value from the CRFConfig constants that control ef tools<br>
